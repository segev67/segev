public class Square3x3 {
    ...
    public void whosThereRow(int row, boolean[] values) {
        if (row < 0 || row >= SIZE || values.length != 10) return;
    ...
    public void whosThereCol(int col, boolean[] values) {
        if (col < 0 || col >= SIZE || values.length != 10) return;

        for (int j = 0; j < SIZE; j++) {
            if (array[j][col] >= 1 && array[j][col] <= 9) {
                values[array[j][col]] = true;
            }
        }
    }
}
